...
import { FiUsers, FiUser, FiMail, FiEdit, FiTrash2, FiPlus, FiRefreshCw, FiCheck, FiX, FiShield, FiFilter, FiChevronDown, FiChevronRight } from 'react-icons/fi';
import { useToast } from '../../hooks/useToast';
import { useAuthStore } from '../../store/authStore';
import { useUsers, useCreateUser, useUpdateUser, useDeleteUser, useClearAllUsers } from '../../hooks/queries/useUsers';
...
export default function AdminUsers() {
  ...
  const [editData, setEditData] = useState<{ role?: 'admin' | 'user'; adminRole?: 'admin' | 'super-admin'; isActive?: boolean; permissions?: string[] }>({});
  const [showAddModal, setShowAddModal] = useState(false);
  const [showConfirmModal, setShowConfirmModal] = useState(false);
  const [confirmAction, setConfirmAction] = useState<{ userId: string; email: string } | null>(null);
  const [expandedModules, setExpandedModules] = useState<string[]>([]); // Track which modules are expanded
  
    // Server State (TanStack Query)
    const { data: rawUsers, isLoading: loading, error, refetch } = useUsers({ 
    ...
    const submitting = createUserMutation.isPending || updateUserMutation.isPending || deleteUserMutation.isPending || clearAllMutation.isPending;

  
  // Permission checks
  const canWrite = hasAccessType('management', 'users', 'write');
  const canDelete = hasAccessType('management', 'users', 'write'); // Delete requires write access
  ...
  const availablePermissions = permissionModules.flatMap(module => module.options);
  ...
  const handleEdit = (user: any) => {
    ...
    setEditData({ 
      role: user.role,
      adminRole: user.adminRole || (user.role === 'admin' ? 'admin' : undefined),
      isActive: user.isActive,
      permissions: user.permissions && Array.isArray(user.permissions) ? [...user.permissions] : ['tools', 'settings']
    });
  };
  ...
  const handleSave = async (userId: string) => {
    try {
      ...
      const finalRole = editData.role !== undefined ? editData.role : userToUpdate.role;
      if (finalRole === 'admin' && editData.adminRole !== undefined && editData.adminRole !== userToUpdate.adminRole) {
        await apiClient.updateUserAdminRole(userId, editData.adminRole);
        updates.push('admin role');
      }
      
      if (editData.isActive !== undefined && editData.isActive !== userToUpdate.isActive) {
        await apiClient.updateUserStatus(userId, editData.isActive);
        updates.push('status');
      }
      
      // Save permissions if the user role is 'user' (admins don't need permissions)
      if (editData.permissions !== undefined && finalRole === 'user') {
        const currentPerms = userToUpdate.permissions && Array.isArray(userToUpdate.permissions) 
          ? userToUpdate.permissions 
          : ['tools', 'settings'];
        const permsChanged = JSON.stringify([...editData.permissions].sort()) !== JSON.stringify([...currentPerms].sort());
  ...
  if (error) {
    const errorMessage =
      error instanceof Error ? error.message : typeof error === 'string' ? error : 'Failed to load users';
    return (
      <div className="flex items-center justify-center py-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button 
              ...
              disabled={!canWrite}
              title={!canWrite ? 'You do not have permission to create users' : 'Add new user'}
            >
                ...
                <TableRow key={user.id || user._id || index}>
                    ...
                    {editingUser === user.id ? (
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          // If changing to admin, clear permissions (admins have full access)
                          if (newRole === 'admin') {
                            setEditData({ ...editData, role: newRole, permissions: [] });
                          } else {
                            // If changing to user, set default permissions if none exist
                            const currentPerms = editData.permissions || user.permissions || ['tools', 'settings'];
                            setEditData({ ...editData, role: newRole, permissions: currentPerms });
                          }
                        }}
                  ...
                  <TableCell className="whitespace-nowrap">
                    {editingUser === user.id ? (
                      ...
                      (editData.role === 'admin' || (editData.role === undefined && user.role === 'admin')) ? (
                        <select
                          value={editData.adminRole || user.adminRole || 'admin'}
                          onChange={(e) => setEditData({ ...editData, adminRole: e.target.value as 'admin' | 'super-admin' })}
                          className="px-2 py-1 text-sm border border-border rounded focus:ring-2 focus:ring-blue-500"
                        >
                          <option value="admin">Admin</option>
                      ...
                      <Badge variant={user.adminRole === 'super-admin' ? 'default' : 'secondary'}>
                        {user.adminRole === 'super-admin' ? 'Super Admin' : 'Admin'}
                      </Badge>
                    ) : (
                  ...
                  <TableCell className="whitespace-nowrap">
                    {editingUser === user.id ? (
                      <select
                        value={editData.isActive !== undefined ? editData.isActive.toString() : user.isActive.toString()}
                  ...
                  <TableCell>
                    {editingUser === user.id ? (
                      ...
                      (editData.role === 'user' || (editData.role === undefined && user.role === 'user')) ? (
                        <div className="space-y-2">
                          <div className="text-xs text-muted-foreground mb-1 font-medium">Select Permissions:</div>
                          <div className="space-y-3 p-3 border border-border rounded-lg bg-background max-w-md">
                            {permissionModules.map((module) => {
                              const currentPerms = editData.permissions !== undefined
                                ? editData.permissions
                                : (user.permissions && Array.isArray(user.permissions) && user.permissions.length > 0
                                  ? user.permissions
                                  : ['tools', 'settings']);

                              const modulePermIds = module.options.map(opt => opt.id);
                              const hasAnyModulePerm = modulePermIds.some(id => currentPerms.includes(id));
                              const hasAllModulePerms = modulePermIds.every(id => currentPerms.includes(id));

                              return (
                                <div key={module.id} className="border border-border rounded-lg bg-white dark:bg-gray-900">
                                  ...
                                  <label className="flex items-center justify-between p-3 cursor-pointer hover:bg-accent transition-colors rounded-t-lg">
                                    <div className="flex items-center gap-2">
                                      <input
                                        ...
                                        onChange={(e) => {
                                          const basePerms = editData.permissions !== undefined
                                            ? editData.permissions
                                            : (user.permissions && Array.isArray(user.permissions) ? user.permissions : ['tools', 'settings']);

                                          let newPerms;
                                          if (e.target.checked) {
                                            // Add all module permissions
                                            newPerms = [...new Set([...basePerms, ...modulePermIds])];
                                          } else {
                                            // Remove all module permissions
                                            newPerms = basePerms.filter((p: string) => !modulePermIds.includes(p));
                                          }
                                    ...
                                    <Badge variant="outline" className="text-xs">
                                      {modulePermIds.filter(id => currentPerms.includes(id)).length}/{modulePermIds.length}
                                    </Badge>
                                  </label>
                                  ...
                                  <div className="px-3 pb-3 pt-1 border-t border-border space-y-1.5">
                                    {module.options.map((option) => (
                                      ...
                                      >
                                        <input
                                          type="checkbox"
                                          checked={currentPerms.includes(option.id)}
                                          onChange={(e) => {
                                            const basePerms = editData.permissions !== undefined
                                              ? editData.permissions
                                              : (user.permissions && Array.isArray(user.permissions) ? user.permissions : ['tools', 'settings']);
                                            ...
                                            let newPerms;
                                            if (e.target.checked) {
                                              newPerms = [...basePerms, option.id];
                                            } else {
                                              newPerms = basePerms.filter((p: string) => p !== option.id);
                                            }
                                            setEditData({ ...editData, permissions: newPerms });
                                          }}
                          ...
                          <p className="text-xs text-muted-foreground mt-1">
                            Selected: {editData.permissions !== undefined
                              ? editData.permissions.length
                              : (user.permissions && Array.isArray(user.permissions) ? user.permissions.length : 2)} permission(s)
                          </p>
                        </div>
                      ) : (
                        <span className="text-xs text-muted-foreground italic">Full Access (Admin)</span>
                      )
                    ) : user.role === 'user' ? (
                      <div className="space-y-1.5">
                        {permissionModules.map((module) => {
                          ...
                          const modulePermIds = module.options.map(opt => opt.id);
                          const activeModulePerms = modulePermIds.filter(id => userPerms.includes(id));
                            ...
                            <div key={module.id} className="flex items-center gap-1.5">
                              ...
                              <div className="flex flex-wrap gap-1">
                                {activeModulePerms.map((permId) => {
                                  const permInfo = module.options.find(p => p.id === permId);
                                  return (
                    ...
                    {editingUser === user.id ? (
                      ...
                      <div className="flex items-center justify-end gap-2">
                        <Button
                          ...
                          title={canWrite ? "Edit" : "You do not have permission to edit users"}
                          disabled={!canWrite}
                        >
                        ...
                        <Button
                          ...
                          size="icon"
                          className="text-red-600 hover:text-red-900"
                          title={canDelete ? "Delete" : "You do not have permission to delete users"}
                          disabled={!canDelete}
                        >
        ...
        {users.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {filterRole || filterStatus ? 'Try adjusting your filters.' : 'Get started by creating a new user.'}
            </p>
          </div>
        )}
      </div>
      ...
      <Dialog open={showAddModal} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6 py-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {newUser.role === 'admin' ? 'Admins have full system access' : 'Users have limited access based on permissions'}
                  </p>
                </div>
              </div>
            ...
            {/* Permissions Section - Only for Users */}
            {newUser.role === 'user' && (
              <div className="space-y-4">
                <Separator />
                <div className="space-y-2">
                  <div className="flex items-center gap-2 mb-2">
                    <h3 className="text-sm font-semibold text-foreground">Sidebar Permissions</h3>
                    <Separator className="flex-1" />
                  </div>
                  <p className="text-sm text-muted-foreground">
                    Select which sections this user can access. The backend will enforce these permissions.
                  </p>
                  
                  <div className="space-y-2 p-4 border border-border rounded-lg bg-muted/30">
                    {permissionModules.map((module) => {
                      const modulePermIds = module.options.map(opt => opt.id);
                      const hasAnyModulePerm = modulePermIds.some(id => newUser.permissions.includes(id));
                      const selectedCount = modulePermIds.filter(id => newUser.permissions.includes(id)).length;
                        ...
                        <div key={module.id} className="border border-border rounded-lg bg-card overflow-hidden">
                          ...
                          <div className="flex items-center justify-between p-3 hover:bg-accent/50 transition-colors">
                            <div className="flex items-center gap-3 flex-1">
                              ...
                              <label className="flex items-center gap-3 cursor-pointer flex-1" onClick={toggleModule}>
                                <input
                                  type="checkbox"
                                  checked={hasAnyModulePerm}
                                  onChange={(e) => {
                                    ...
                                    if (e.target.checked) {
                                      // Add all module permissions
                                      newPerms = [...new Set([...newUser.permissions, ...modulePermIds])];
                                    } else {
                                      // Remove all module permissions
                                      newPerms = newUser.permissions.filter(p => !modulePermIds.includes(p));
                                    }
                          ...
                          {/* Module Pages - Only show when expanded */}
                          {isExpanded && (
                            <div className="px-3 pb-3 pt-1 border-t border-border space-y-1.5 bg-muted/20">
                              {module.options.map((option) => (
                                <label
                                  key={option.id}
                                ...
                                >
                                  <input
                                    type="checkbox"
                                    checked={newUser.permissions.includes(option.id)}
                                    onChange={(e) => {
                                      let newPerms;
                                      if (e.target.checked) {
                                        newPerms = [...newUser.permissions, option.id];
                                      } else {
                                        newPerms = newUser.permissions.filter(p => p !== option.id);
                                      }
                                      setNewUser({ ...newUser, permissions: newPerms });
                                    }}
                                    className="w-4 h-4 text-primary border-border rounded focus:ring-2 focus:ring-primary"
                  ...
                  <p className="text-xs text-muted-foreground">
                    <strong>Note:</strong> No default permissions. Users will only see selected options in their sidebar. 
                    The backend will enforce these permissions on all API calls.
                  </p>
                </div>
              </div>
            )}
